...
\begin{figure}[ht!]
    ...
    \caption{Jenson's Inequality}
    \label{fig:jenson-inequality}
\end{figure}
...
To visualise \autoref{thm:jenson} (Jenson's Inequality), \autoref{fig:jenson-inequality} considers a uniform distribution over \(\brce{x_1, x_2, x_3, x_4}\), and the centre of the quadrilateral is marked red, with \(y\)-coordinate \(\Expt\brac{f\para{X}}\), while \(f\para{\Expt\brac{X}}\) is marked as the blue dot on the curve.
...
\begin{proof}
    ...
    \[
        m = tx + \para{1 - t}y
    \]
    for some \(t \in \ccintv{0}{1}\). This gives
    \[
...
Now we use \autoref{lem:convex-fn-tangents} to prove \autoref{thm:jenson} (Jenson's Inequality).

\begin{proof}[of \autoref{thm:jenson} (Jenson's Inequality)]
