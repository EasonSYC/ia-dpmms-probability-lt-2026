...
This statement is relatively long to prove (and is non-examinable), but a statement that is also useful is given below, which is a corollary of the above statement.
...
This leads to \autoref{prop:inclusion-exclusion}, the Inclusion--Exclusion Formula.

\begin{proposition}[Inclusion--Exclusion Formula]
    \label{prop:inclusion-exclusion}%%
...
\begin{remark}
    ...
    Taking \(A_1, A_2, \ldots, A_n \in \mathcal{F}\), applying \autoref{prop:inclusion-exclusion} (Inclusion--Exclusion Formula) to \(\Prob\para{\bigcup_{i = 1}^{n} A_i}\), we have
    \[
...
\begin{proof}
    ...
    \[
        \Prob\para{\bigcup_{i = 1}^{n} A_i} = \Prob\para{i = 1}^{n - 1} A_i + \Prob\para{A_n} - \Prob\para{B_1 \cup B_2 \cup \cdots \cup B_{n - 1}}.
    \]
