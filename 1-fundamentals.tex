...
\begin{examples}[Equally likely outcomes]
    \label{ex:equally-likely-outcomes}%
    \begin{enumerate}
...
\begin{proof}
    ...
    \end{align*}
    and we apply \autoref{thm:law-total-probability} for the numerator.
\end{proof}
...
\begin{example}[False Positives for a Rare Condition]
    ...
    Hence, using \autoref{cor:bayes} (Bayes' Formula), we have
    \begin{align*}
    ...
    \[
        \Prob\Conditional{A}{P} = \frac{1}{1 + \frac{\Prob\Conditional{P}{A\compl} \cdot \Prob\para{A\compl}}{\Prob\Conditional{P}{A} \cdot \Prob{A}}}.
    \]

    We have \(\Prob\para{A\compl}\) and \(\Prob\Conditional{P}{A}\) are both very close to \(1\), so the relevant ratio simplifies to \(\frac{\Prob\Conditional{P}{A\compl}}{A}\). But
    \[
